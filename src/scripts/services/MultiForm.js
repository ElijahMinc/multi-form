...
export class MultiForm extends Form {
   ...
   constructor(formId, options = {
      ...
      this.currentStep = this.options?.currentStep ?? this.$steps.findIndex(e => 
   ...
   async handleSteps(e){
      ...
      const getValuesByStep = async (step) => {
         ...
         const validateInputCb =(inputName, inputValue) => {
            ...
            } catch (er) { // or we can THROW error 
               return false
            }
         };
  ...
  getValidateMultiForm() {
   ...
   function checkRequiredFieldsByCurrentStep(){
      const requiredFields = currentStepContainer.querySelectorAll('._require');
         const filledRequiredFields = Array.from(requiredFields)
            .every(field =>  field.hasAttribute('data-success'));
